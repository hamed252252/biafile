...
export default async function GradePage({ params }: { params: Promise<RouteParams> }) {
  const { degree, grade } = await params;

  const { lessons, classData } = await fetchDataLessonsClassData();

  // بررسی وجود degree
  const degreeData = classData.entities.find((d) => matchesSlug(degree, d));
  if (!degreeData) return notFound();

  // بررسی وجود grade
  const gradeData = degreeData.subResultCategorys.find((g) => matchesSlug(grade, g));
