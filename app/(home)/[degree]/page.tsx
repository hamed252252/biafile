...
export default async function DegreePage({ params }: PageProps) {
  ...
  const raw = await fetch('https://api.biafile.ir/Api/Categorys/Public').then((r) => r.json());
  const classData: LocalClassData = {
    entities: raw.entities,
  };

  // 3) پیدا کردن دادهٔ مربوط به degree
  const degreeData = classData.entities.find((c) => c.uniqCode === degree);
  ...
  return (
    <div className="mx-auto py-10" dir="rtl">
      <h1 className="text-4xl font-bold mb-8 text-right">{degreeData.title}</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4 px-4">
        {degreeData.subResultCategorys.map((subitem) => {
          // درس‌های هر زیرشاخه
          const lessons: LessonLink[] = subitem.subResultCategorys.map((lesson) => ({
            name: lesson.title,
