...
export async function fetchDataLessonsClassData(): Promise<{
  ...
  const [lRes, cRes] = await Promise.all([
    fetch('https://api.biafile.ir/Api/LessonHeadings/AllForPublicPage'),
    fetch('https://api.biafile.ir/Api/Categorys/Public'),
  ]);
  if (!lRes.ok || !cRes.ok) throw new Error('fetch failed');
  const lessons = (await lRes.json()) as ApiResponseLessonHeading;
