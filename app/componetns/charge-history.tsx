...
const totalCharges = chargeHistory.reduce((acc, item) => acc + item.amount, 0);
...
export default function ChargeHistoryChart() {
  return (
    <div dir="rtl" className="flex flex-col lg:flex-row mt-10 p-4 gap-6">
      {/* Line Chart */}
      <Card className="flex-1">
        <CardHeader>
          <CardTitle className="text-xl font-semibold">نمودار خطی شارژ</CardTitle>
            ...
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <YAxis
                  ...
                  axisLine={false}
                  tickMargin={8}
                  tickFormatter={(value) => Intl.NumberFormat('fa-IR').format(value)}
                />
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      const { date, amount, status } = payload[0].payload;
                      return (
                        <div className="rounded-md bg-white p-2 shadow-lg">
                          ...
                          <p className="text-sm text-muted-foreground">{`مبلغ: ${Intl.NumberFormat(
                            'fa-IR',
                          ).format(amount)} تومان`}</p>
                          <p
                            className={`text-sm ${
                              status === 'موفق' ? 'text-green-600' : 'text-red-600'
                            }`}
                          >
                            {status === 'موفق' ? 'وضعیت: موفق' : 'وضعیت: ناموفق'}
                          </p>
                        </div>
                      );
                    }
      ...
      <Card className="flex-1">
        <CardHeader>
          <CardTitle className="text-xl font-semibold">نمودار میله‌ای شارژ</CardTitle>
          <CardDescription className="text-muted-foreground">
      ...
      <Card className="flex-1">
        <CardHeader>
          <CardTitle className="text-xl font-semibold">لیست تراکنش‌ها</CardTitle>
          <CardDescription className="text-muted-foreground">
          ...
          <ScrollArea className="h-[300px]">
            <ul className="space-y-4">
              {chargeHistory.map((charge, index) => (
                ...
                >
                  <div className="text-sm font-medium">{charge.date}</div>
                  <div className="text-sm">
                    {Intl.NumberFormat('fa-IR').format(charge.amount)} تومان
                  </div>
                  <div
                    className={`text-sm ${
                      charge.status === 'موفق' ? 'text-green-600' : 'text-red-600'
                    }`}
                  >
                    {charge.status}
                  </div>
