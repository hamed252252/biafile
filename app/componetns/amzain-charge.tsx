...
const initialTransactionData: TransactionData[] = [
  ...
  },
  {
    ...
    amount: 30000,
    expiryDate: new Date(new Date().getTime() + 25 * 24 * 60 * 60 * 1000),
  },
  {
    id: 4,
    date: '۱۴۰۲/۰۳/۱۰',
  ...
  },
  {
    ...
    description: 'اعتبار مستقیم',
    amount: 10000,
    expiryDate: new Date(new Date().getTime() + 20 * 24 * 60 * 60 * 1000),
  },
];
...
function Countdown({ expiryDate }: CountdownProps) {
  ...
  return (
    <div className="grid grid-cols-4 gap-1 text-center">
      {['روز', 'ساعت', 'دقیقه', 'ثانیه'].map((unit, index) => (
        <div key={unit} className="bg-primary text-xs text-primary-foreground rounded-full p-1.5">
          ...
          <div className="text-[0.5rem] flex justify-center items-center">{unit}</div>
        </div>
      ))}
    </div>
  );
...
export default function Component() {
  ...
  const handleRecharge = () => {
    ...
    setTransactionData([newTransaction, ...transactionData.slice(0, 4)]);
    setPhoneNumber('');
    setAmount('');
  };

  return (
    <div dir="rtl" className="container mx-auto p-4 space-y-6 font-sans">
      <h1 className="text-3xl font-bold text-center mb-6">اعتبار هدیه</h1>

      <Card>
        <CardHeader>
          <CardTitle>موجودی اعتبار</CardTitle>
          <CardDescription>روند موجودی اعتبار در شش ماه گذشته</CardDescription>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={rechargeData}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="balance" stroke="#8884d8" name="موجودی" />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>پنج تراکنش آخر حساب شما</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">شمارش معکوس</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <TableRow key={transaction.id}>
                  ...
                  <TableCell className="text-center ">
                    {transaction.expiryDate ? (
                      <Countdown expiryDate={transaction.expiryDate} />
                    ) : (
                      '-'
                    )}
                  </TableCell>
