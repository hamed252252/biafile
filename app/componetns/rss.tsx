...
export default function NewsletterForm() {
  ...
  // email regex
  const isValid = (v: string) => /^[\w-.]+@([\w-]+\.)+[\w-]{2,}$/.test(v.trim());

  // fetch existing
  const fetchExisting = useCallback(async () => {
    setInitLoading(true);
    try {
      const res = await fetch('https://api.biafile.ir/Api/NewsLetters/AllForPublicPage');
      if (!res.ok) throw new Error('Network error');
      const data = (await res.json()) as NewsletterResponse;
      if (data.status === 'success' && Array.isArray(data.entities)) {
        setExisting(data.entities.map((e) => e.email.toLowerCase()));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const jDate = `${jy}/${jm.toString().padStart(2, '0')}/${jd.toString().padStart(2, '0')}`;
    ...
    const payload: NewsletterPayload = {
      registerDate: jDate,
      editDate: jDate,
      uniqCode: (crypto as any).randomUUID?.() || Math.random().toString(36).slice(2),
      registerTime: time,
      editTime: time,
      jsonLableTexts: '',
      resultJsonLables: [],
    ...
    try {
      const res = await fetch('https://api.biafile.ir/Api/NewsLetters', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok || result.status !== 'success') throw new Error(result.message || 'خطای سرور');
      setMessage({
        type: 'success',
      ...
      setMessage({
        type: 'error',
        text: err.message || 'ثبت ایمیل ناموفق بود.',
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            ...
            <span>آخرین اخبار و بروزرسانی‌ها</span>
          </motion.div>
          <motion.h1
          ...
          >
            همیشه اولین نفری باشید که از آخرین اخبار، محصولات و پیشنهادات ویژه ما مطلع می‌شوید
          </motion.p>
        </motion.div>
          ...
          >
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 grid grid-cols-2 gap-4">
                <AnimatedCounter value={count} label="مشترکین" />
                <AnimatedCounter value={24} label="خبرنامه در ماه" />
              </div>
            </div>
          </motion.div>
            ...
            <AnimatePresence mode="wait">
              {successView ? (
                ...
                >
                  ...
                  >
                    از اینکه به جمع مشترکین ما پیوستید متشکریم. منتظر دریافت اخبار و بروزرسانی‌های
                    ما باشید.
                  </motion.p>
                  <motion.button
                    ...
                    <>
                          ...
                          <motion.div variants={{ itemVariants }} className="space-y-2">
                            ...
                            <AnimatePresence>
                              {error && (
                                ...
                                >
                                  <AlertCircle className="h-4 w-4 ml-1" /> {error}
                                </motion.p>
                              )}
                            </AnimatePresence>
                          </motion.div>
                      ...
                      >
                        <h3 className="text-lg font-semibold mb-4 flex items-center text-gray-900 dark:text-white">
                          <Star className="h-5 w-5 text-yellow-500 ml-2" /> مزایای عضویت
                        </h3>
                      ...
                      >
                        <p>
                          ما به حریم خصوصی شما احترام می‌گذاریم. شما می‌توانید هر زمان که بخواهید از
                          خبرنامه خارج شوید.
                        </p>
                      </motion.div>
                    </>
        ...
        >
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-800">
            ...
            >
              ...
              <span dir="ltr" className="font-bold text-blue-600 dark:text-blue-400 mr-2">
                {count.toLocaleString()}
              </span>
            </motion.div>
          </div>
