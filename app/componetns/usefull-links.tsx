...
import { Search, ExternalLink, Copy, Check, LinkIcon, X, Bookmark, Filter } from 'lucide-react';
...
const UsefulLinks: React.FC = () => {
    ...
    (async () => {
      try {
        const [groupRes, linkRes] = await Promise.all([
          fetch('https://api.biafile.ir/Api/GroupLinks/AllForPublicPage'),
          fetch('https://api.biafile.ir/Api/Links/AllForPublicPage'),
        ]);
        if (!groupRes.ok || !linkRes.ok) throw new Error('Network error');

        const { entities: groupEntities } = await groupRes.json();
        const { entities: linkEntities } = await linkRes.json();
  ...
  const filteredLinks = useMemo(
    () =>
      links.filter((link) => {
        ...
        const groupMatch = selectedGroupId === null || link.groupLinkID === selectedGroupId;
  ...
  const groupCounts = useMemo(() => {
    return links.reduce<Record<number, number>>((acc, link) => {
      acc[link.groupLinkID] = (acc[link.groupLinkID] || 0) + 1;
      return acc;
    }, {});
  }, [links]);

  const handleCopyLink = useCallback((url: string, id: number) => {
    ...
    >
      <CardHeader className="relative z-10 px-6 pt-6 pb-0">
        <div className="flex justify-between items-center">
          ...
          <Badge variant="outline" className="px-3 py-1 bg-background/80 backdrop-blur-sm">
            {loading ? <Skeleton className="h-4 w-16" /> : <span>{links.length} لینک</span>}
          </Badge>
        </div>
      </CardHeader>
          ...
          {showFilters && (
            ...
            >
              <h3 className="text-sm font-medium mb-3 flex items-center gap-2">
                <Bookmark className="w-4 h-4" /> فیلتر بر اساس دسته‌بندی
              </h3>
              <div className="flex flex-wrap gap-2">
                ...
                >
                  همه
                  <Badge variant="secondary" className="mr-2 bg-background/50">
                    {links.length}
                  </Badge>
                </Button>
                {loading
                  ? Array.from({
                      length: 5,
                    }).map((_, i) => <Skeleton key={i} className="h-9 w-24 rounded-full" />)
                  : groups.map((group) => (
                      <Button
                        key={group.id}
                        size="sm"
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              لطفاً با کلمات کلیدی دیگری جستجو کنید یا فیلترها را تغییر دهید.
            </p>
            {searchTerm && (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <h3 className="text-lg font-semibold group-hover:text-primary transition-colors flex items-center gap-2">
                          <LinkIcon className="w-4 h-4 opacity-70" /> {link.title}
                        </h3>
                        <Badge variant="secondary" className="bg-secondary/50">
                          {groups.find((g) => g.id === link.groupLinkID)?.title}
                        </Badge>
                      </div>
                      <div
      ...
      <CardFooter className="relative z-10 flex justify-between items-center text-xs text-muted-foreground p-4 border-t">
        ...
        {!loading && filteredLinks.length > 0 && (
          <span>
            نمایش {filteredLinks.length} از {links.length} لینک
          </span>
        )}
      </CardFooter>
    </Card>
