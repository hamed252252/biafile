...
const GradeCard: React.FC<GradeCardProps> = ({ degree, gradeData, lessons, gradeSlug }) => {
  return (
    <div dir="rtl" className="container mx-auto p-4 rtl">
      <Card className="mb-8 shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <CardHeader className="bg-gradient-to-l from-primary to-primary/75 text-white p-8">
          <CardTitle className="text-3xl font-bold mb-4">{gradeData.title}</CardTitle>
        ...
        <CardContent className="p-6">
          <Tabs
            dir="rtl"
            defaultValue={gradeData.subResultCategorys[0]?.uniqCode}
            className="w-full"
          >
            <TabsList className="flex justify-start overflow-x-auto overflow-y-hidden mb-6 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
              {gradeData.subResultCategorys.map((subCategory) => (
                <TabsTrigger
                  key={subCategory.id}
                  value={subCategory.uniqCode}
            ...
            {gradeData.subResultCategorys.map((subCategory) => (
              <TabsContent dir="rtl" key={subCategory.id} value={subCategory.uniqCode}>
                <h4 className="text-2xl font-semibold mb-6 text-primary border-r-4 border-primary pr-4">
                  <Link href={`${gradeSlug}/${subCategory.uniqCode}`}>{subCategory.title}</Link>
                </h4>
                <ScrollArea dir="rtl" className="h-[calc(100vh-100px)]">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {lessons
                      .filter((lesson) => lesson.categoryID === subCategory.id)
                      .map((lesson) => (
                        <LessonCard
                          key={lesson.id}
                          subject={subCategory.uniqCode}
                          lesson={lesson}
                          degree={degree}
                          gradeSlug={gradeSlug}
                        />
...
const LessonCard: React.FC<LessonCardProps> = ({ subject, lesson, degree, gradeSlug }) => {
  return (
    <Card className="flex flex-col h-full shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden group">
      <div className="relative w-full h-48 overflow-hidden">
        {lesson.imageUIrl ? (
          <Image
            src={lesson.imageUIrl || '/placeholder.svg'}
            alt={lesson.title}
            layout="fill"
            objectFit="cover"
      ...
      <CardContent suppressHydrationWarning className="flex-grow">
        <div
      ...
      <CardFooter className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
        <Link href={`/${degree}/${gradeSlug}/${subject}/${lesson.id}`} passHref className="w-full">
