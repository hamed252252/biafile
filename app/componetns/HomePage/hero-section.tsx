...
const IMAGE_BASE_URL = 'https://api.biafile.ir/Uploadfiles/Files';
...
export default function HeroSection() {
  ...
  useEffect(() => {
    fetch('https://api.biafile.ir/Api/Herosections/AllForPublicPage')
      .then((res) => {
        if (!res.ok) throw new Error('خطا در دریافت اطلاعات');
        return res.json();
      })
      .then((data) => {
        const item = Array.isArray(data.entities) ? data.entities[0] : null;
        if (item) setHero(item as HeroEntity);
        else setError('هیچ داده‌ای یافت نشد');
      })
      .catch(() => setError('اتصال به سرور امکان‌پذیر نیست'))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <div className="py-12 text-center">در حال بارگذاری...</div>;
  if (error || !hero)
    return <div className="py-12 text-center text-red-500">{error || 'خطایی رخ داده'}</div>;
    ...
    <section className="relative w-full overflow-hidden bg-white dark:bg-gray-900">
      ...
      <div className="relative z-10 py-16 md:py-24 container mx-auto px-4 flex flex-col md:flex-row items-center gap-8">
        ...
        <div className=" w-full h-full md:w-1/2">
          {mediaItems.length > 1 ? (
            <Carousel dir="ltr" className="relative rounded-2xl overflow-hidden shadow-xl">
              <CarouselContent className="space-x-0">
                {mediaItems.map((item, idx) => (
                  <CarouselItem key={idx} className="relative w-full aspect-[3/2]">
                    {item.type === 'image' ? (
                      <Image
